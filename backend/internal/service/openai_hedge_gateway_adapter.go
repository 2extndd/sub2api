...
func boundedOpenAIHedgeForwardError(err error) error {
	...
	if errors.As(err, &failoverErr) && failoverErr != nil {
		// Preserve the bounded policy metadata consumed by the outer account retry
		// loop, but never carry raw provider data across the hedge coordinator.
		bounded := *failoverErr
		bounded.ResponseBody = nil
		bounded.ResponseHeaders = nil
		bounded.ClientMessage = ""
		return &bounded
	}
	return errors.New("openai hedge upstream failed")
}
