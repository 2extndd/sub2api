<template>
  <AppLayout>
        ...
        <div class="py-1">
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <!-- Upstream account deny-list -->
              <button
                @click="handleAccountDenials(user); closeActionMenu()"
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
              >
                <Icon name="shield" size="sm" class="text-gray-400" :stroke-width="2" />
                {{ t('admin.users.accountDenialsAction') }}
              </button>
    ...
    <UserAccountDenialsModal :show="showAccountDenialsModal" :user="accountDenialsUser" @close="closeAccountDenialsModal" />
...
import UserAccountDenialsModal from '@/components/admin/user/UserAccountDenialsModal.vue'
...
const showAccountDenialsModal = ref(false)
const accountDenialsUser = ref<AdminUser | null>(null)
...
const handleAccountDenials = (user: AdminUser) => {
  accountDenialsUser.value = user
  showAccountDenialsModal.value = true
}

const closeAccountDenialsModal = () => {
  showAccountDenialsModal.value = false
  accountDenialsUser.value = null
}
