...
func TestBoundedOpenAIHedgeForwardErrorPreservesRetryPolicyWithoutRawProviderData(t *testing.T) {
	source := &UpstreamFailoverError{
		StatusCode:             http.StatusTooManyRequests,
		ResponseBody:           []byte(`{"error":"secret provider response"}`),
		...
		ClientMessage:          "sensitive client message",
		FailureClass:           GatewayFailureRateLimited,
		RetryDisposition:       GatewayRetryNextAccount,
		StatusKnown:            true,
	}

	err := boundedOpenAIHedgeForwardError(source)
	var bounded *UpstreamFailoverError
	require.ErrorAs(t, err, &bounded)
	require.NotSame(t, source, bounded)
	require.Equal(t, source.StatusCode, bounded.StatusCode)
	require.Equal(t, source.RetryableOnSameAccount, bounded.RetryableOnSameAccount)
	require.Equal(t, source.Stage, bounded.Stage)
	require.Equal(t, source.Scope, bounded.Scope)
	require.Equal(t, source.Reason, bounded.Reason)
	require.Equal(t, source.NextAccountAction, bounded.NextAccountAction)
	require.Equal(t, source.FailureClass, bounded.FailureClass)
	require.Equal(t, source.RetryDisposition, bounded.RetryDisposition)
	require.Equal(t, source.StatusKnown, bounded.StatusKnown)
	require.Nil(t, bounded.ResponseBody)
	require.Nil(t, bounded.ResponseHeaders)
	require.Empty(t, bounded.ClientMessage)
	require.NotContains(t, err.Error(), "secret")

	coordinatorErr := errors.Join(
		ErrOpenAIHedgeAllAttemptsFailed,
		fmt.Errorf("primary attempt failed: %w", err),
	)
	var retryLoopErr *UpstreamFailoverError
	require.ErrorAs(t, coordinatorErr, &retryLoopErr)
	require.True(t, retryLoopErr.ShouldRetryNextAccount())
}
