export default {
    accounts: {
      ...
      dataImport: '导入',
      moreActions: '更多操作',
      denyingUsersAction: '查看排除此账号的用户',
      denyingUsersShort: '排除用户',
      denyingUsersTitle: '用户排除列表',
      denyingUsersHint: '只读显示上游账号排除列表中包含此账号的用户。',
      denyingUsersEmpty: '没有用户明确排除此账号。',
      denyingUsersLoadError: '加载排除此账号的用户失败',
      dataActions: '数据操作',
      toolActions: '工具',
      viewColumns: '列显示',
      ...
      openai: {
        ...
        endpointCapabilitiesDesc:
          '用于调度筛选。文本端点会跟随上方 Responses API 支持显示为 Responses、Chat Completions 或自动模式；Embeddings 独立控制 /v1/embeddings；图片生成独立控制 /v1/images。',
        ...
        capabilityImages: '图片生成',
        imageGenerationOnly: '仅图片生成',
        imageGenerationOnlyDesc: '仅启用图片生成端点，其他端点将被禁用。图片专用账号不支持 Responses 模式配置。',
        responsesStatusAutoSupported: '自动探测：Responses',
        responsesStatusAutoUnsupported: '自动探测：Chat Completions',
        responsesModeTextDisabledHint: '图片专用账号不支持此设置。请启用文本生成（Chat Completions 或 Responses）来配置响应模式。',
      ...
      billingRateMultiplier: '账号成本倍率',
      billingRateMultiplierHint: '仅用于上游成本和利润核算，不影响用户扣费',
      usageBillingMultiplier: '用户用量扣费倍率',
      usageBillingMultiplierHint: '该路由账号成功后仅应用一次；1 表示保持官方用户价格',
      usageBillingMultiplierInvalid: '用户用量扣费倍率必须是有限数、大于 0 且不超过 1,000,000',
      expiresAt: '过期时间',
