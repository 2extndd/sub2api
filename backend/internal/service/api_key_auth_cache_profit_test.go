...
func TestAPIKeyAuthSnapshotProfitControlRoundtrip(t *testing.T) {
	...
	require.Equal(t, 24, snapshot.Version, "v24 snapshot retains profit and denial policy metadata while adding the enforced group model allowlist")
	require.GreaterOrEqual(t, snapshot.Version, 19, "v19 起认证快照携带用户拒绝账号列表")
